...
{
    // this code was directly copy and pasted from another plugin, and it doesn't crash, so i assume this is okay
...
#pragma warning disable CS8604 // Possible null reference argument.
    public static class ZoneInfoHandler
    {
        public static void Init()
        {
            ...
            ExcelSheet<TerritoryType> territorySheet = WaymarkOCD.dataManager.GetExcelSheet<TerritoryType>();
            ExcelSheet<ContentFinderCondition> contentFinderSheet = WaymarkOCD.dataManager.GetExcelSheet<ContentFinderCondition>();
            ExcelSheet<Map> mapSheet = WaymarkOCD.dataManager.GetExcelSheet<Map>();
        ...
        {
            if (mZoneInfoDict.TryGetValue(ID, out ZoneInfo value))
            {
                return value;
            }
            else
        ...
        public static ZoneInfo GetZoneInfoFromTerritoryTypeID(uint ID)
        {
            ...
            if (mZoneInfoDict.TryGetValue(contentFinderID, out ZoneInfo value))
            {
                return value;
            }
            else
        ...
        {
            if (mTerritoryTypeIDToContentFinderIDDict.TryGetValue(ID, out ushort value))
            {
                return value;
            }
            else
        ...
        public static MapInfo[] GetMapInfoFromTerritoryTypeID(uint ID)
        {
            ...
            if (mMapInfoDict.TryGetValue(mapBaseName, out List<MapInfo> value))
            {
